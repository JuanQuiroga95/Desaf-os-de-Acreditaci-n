  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <header className="mb-12 flex justify-between items-end">
        <div>
          <h1 className="text-4xl font-black tracking-tight mb-2 uppercase italic">Panel Docente</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
        <div className="lg:col-span-2 space-y-8">
          <section className="bg-card border border-border rounded-[2rem] p-8 shadow-sm">
            <h2 className="text-xl font-bold mb-6 flex items-center gap-2 italic uppercase text-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                ...
                <div key={i} className="p-6 rounded-2xl bg-secondary/30 border border-border hover:border-primary/50 transition-all group shadow-sm">
                  <h3 className="font-bold text-lg mb-4 group-hover:text-primary transition-colors">{sub.name}</h3>
                  <div className="flex gap-6">
                    <div className="flex flex-col">
                      <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest leading-none mb-1">Alumnos</span>
                      <span className="text-2xl font-black">{sub.students}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest leading-none mb-1">Desafíos</span>
                      <span className="text-2xl font-black">{sub.tasks}</span>
                    </div>
                  </div>
                  <button className="mt-6 w-full py-3 rounded-xl bg-card border border-border text-[10px] font-black uppercase tracking-[0.2em] hover:bg-primary hover:text-white transition-all flex items-center justify-center gap-2">
                    Gestionar Clase
                    <ArrowRight size={14} />
                  </button>
                </div>
          ...
          <section className="bg-card border border-border rounded-[2rem] p-8 shadow-sm">
            ...
            <div className="space-y-4">
              {[
                ...
                <div key={i} className="flex items-center justify-between p-5 bg-secondary/20 rounded-2xl border border-border/50 hover:bg-secondary/40 transition-all">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">{entry.task}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] text-muted-foreground font-bold uppercase mb-2 tracking-widest">{entry.date}</p>
                    <button className="text-primary text-[10px] font-black uppercase tracking-[0.2em] hover:underline">Corregir Ahora</button>
                  </div>
                </div>
              ))}
            </div>
